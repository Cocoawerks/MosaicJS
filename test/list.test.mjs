...
const { ListItem, ListView, ProgressiveListView } =
...
function list(Type = ListView, props = {}, count = 3) {
  ...
  const view = mount(Type, host, {
    ...props,
    children: [h(PersonItem, {})],
  }).view;
  ...
  return {
    ...
    rows: () =>
      el
        ...
        .filter((s) => s.getAttribute("class")?.includes("person")),
  };
}
...
test("a list draws a row per thing it holds, in the kind of row it was given", () => {
  ...
  assert.deepEqual(
    ...
    ["0: Person 0", "1: Person 1", "2: Person 2"],
  );
});
...
test("each row is handed its own, and where it sits", () => {
  ...
  const lines = el
    ...
    .filter((d) => classesOf(d).includes("v-List-item"));
...
function sized(view, { height = 100, scrollTop = 0 } = {}) {
  Object.defineProperty(view.scroller, "offsetHeight", {
    value: height,
    configurable: true,
  });
...
test("and draws only the rows in view, with a little either side", () => {
  ...
  assert.equal(
    view.totalHeight,
    10000,
    "though it is as tall as all five hundred",
  );
});
...
test("rows of different heights are placed by what each is worth", () => {
  const { view, el } = list(
    ProgressiveListView,
    {
      itemHeight: (index) => (index % 2 === 0 ? 10 : 30),
      extension: "0",
      batch: "0",
    },
    10,
...
test("the reach either side is what the markup asked for", () => {
  const { view, rows } = list(
    ProgressiveListView,
    { itemHeight: "20", extension: "40", batch: "2" },
    500,
  );
