...
const {ToolBar, ToolBarFlex, ToolBarItem} = await import(
...
const overflowOf = (el) =>
    el.childNodes.find(
        (node) => node.nodeType === 1 && classesOf(node).includes("v-ToolBar-overflow"),
    );
...
test("a flexible gap is drawn, so what follows it can be pushed along", () => {
    ...
    assert.equal(gap.getAttribute("aria-hidden"), "true", "and says it reads as nothing");
});
...
test("a hidden item is left in the bar and taken out of the layout", () => {
    ...
    assert.deepEqual(items.filter(showing).map((i) => i.textContent), [
        "New",
        "Open",
        "Share",
    ]);
...
test("nothing overflows while everything fits", () => {
    ...
    assert.equal(showing(overflowOf(el)), false, "the overflow button is not there");
...
test("the items that no longer fit leave from the trailing edge", () => {
    ...
    assert.deepEqual(
        itemsOf(el).filter(showing).map((item) => item.textContent),
        ["New", "Open"],
        "the leading items survive longest",
    );
    assert.equal(showing(overflowOf(el)), true, "and the overflow button is there");
});
...
test("a hidden item is not in the bar, so it is not in the menu either", () => {
    ...
    assert.deepEqual(view.menu.items.map((item) => item.props.text), ["Save"]);
});
