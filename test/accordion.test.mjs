...
const {AccordionSection, AccordionView, Box} = await import(
...
const keyDown = (el, key, target = el) => {
    let prevented = false;
    el.dispatchEvent({type: "keydown", key, target, preventDefault: () => (prevented = true)});
    return prevented;
};
...
const someSections = () => [
    h(AccordionSection, {title: "Delivery", value: "delivery", expanded: "true"},
        h("p", {}, "where it goes")),
    h(AccordionSection, {title: "Payment", value: "payment"}, h("input", {})),
    h(AccordionSection, {title: "Notes", value: "notes"}, h("p", {}, "anything else")),
];
...
test("pressing a header opens that section, and says so", () => {
    ...
    click(headerOf(1));
    assert.deepEqual(openOnes(panels()), ["Delivery", "Payment"], "more than one may be open");
