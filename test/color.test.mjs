...
const {Color, ColorChooserPanel, ColorWell} = await import(
...
test("and is read from hex in any of the three lengths", () => {
    ...
    assert.ok(Color.fromHex("3584e4").equals(new Color(53, 132, 228)), "the hash is optional");
    assert.ok(Color.fromHex("#0af").equals(new Color(0, 170, 255)), "short form doubles each digit");
    assert.equal(Color.fromHex("#3584E480").alpha, 50, "eight digits carry the opacity");
    assert.equal(Color.fromHex("#3584e4", 40).alpha, 40, "six take the one they are given");
});
...
test("hue, saturation and brightness go round and come back", () => {
    for (const [r, g, b] of [[53, 132, 228], [255, 0, 0], [0, 255, 0], [18, 52, 86], [200, 200, 200]]) {
        const colour = new Color(r, g, b);
        const {hue, saturation, brightness} = colour.toHSB();
        assert.ok(Color.fromHSB(hue, saturation, brightness).equals(colour), `${r},${g},${b}`);
    }
});
...
test("the chooser draws the palette, the pickers and the controls", () => {
    ...
    assert.equal(swatches(el).length, 45, "nine families across, five shades down");
    assert.equal(el.querySelectorAll("canvas").length, 2, "the square and the strip");
...
test("choosing a colour reports it", () => {
    const said = [];
    const {view} = chooser({color: "#000000", action: (panel, colour) => said.push(colour)});
...
test("the hex field takes a colour, and ignores what is not one", () => {
    ...
    assert.ok(view.color.equals(new Color(53, 132, 228)), "half-typed text changes nothing");
});
...
test("a grey does not throw the hue away", () => {
    ...
    assert.equal(view.hue, red, "the hue being worked survives a colour that has none");
});
...
test("setting its colour repaints the swatch and reports", () => {
    const said = [];
    const {el, view} = well({color: "#000000", action: (w, colour) => said.push(colour)});

    view.setColor(new Color(255, 0, 0));
    assert.equal(el.querySelectorAll("i")[0].style.backgroundColor, "rgba(255, 0, 0, 1)");
    assert.deepEqual(said.map((c) => c.toHexString()), ["#FF0000"]);
...
test("pressing it opens a chooser, and pressing it again does not build another", () => {
    ...
    assert.ok(view.chooser.color.equals(new Color(53, 132, 228)), "showing the well's colour");
...
test("what the chooser reports, the well takes and passes on", () => {
    const said = [];
    const {view} = well({color: "#000000", action: (w, colour) => said.push(colour)});

    view.click({});
    view.chooser.setColor(new Color(53, 132, 228));

    assert.ok(view.color.equals(new Color(53, 132, 228)));
    assert.deepEqual(said.map((c) => c.toHexString()), ["#3584E4"]);
});
