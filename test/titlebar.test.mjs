...
const {TitleBar, TitleBarButton} = await import(
...
function titlebar(props = {}, children = []) {
    ...
    const region = (name) =>
        el.childNodes.find((node) => classesOf(node).includes(`v-TitleBar-${name}`));
    return {
        host,
        view,
        el,
...
test("the strip stops shrinking where the title would meet a side region", () => {
    const {view, el, actions, title, trailing} = titlebar({title: "Settings"});
...
test("a title bar button is a button wearing the bar's face", () => {
    ...
    assert.ok(classesOf(button).includes("v-TitleBarButton"), "of the bar's kind");
});

test("a momentary button fires its action when it is clicked", () => {
    const fired = [];
    const {trailing} = titlebar({}, [
        h(TitleBarButton, {slot: "trailing", text: "Ada", action: (b) => fired.push(b.text)}),
    ]);
...
const {MenuBarItem, MenuItem} = await import(
    "../examples/Counter_component/build/node_modules/mosaic/frameworks/ui/index.js"
    );
...
function menuBar(count = 2, props = {}) {
    ...
    const {trailing, actions, view, el} = titlebar({}, [
        ...Array.from({length: count}, (_, i) =>
            ...
            h(MenuBarItem, {slot: "actions", text: `Menu ${i}`, ...props}, ...someLines()),
        ),
    ]);
...
test("a menu bar item is a title bar button that says it drops a menu", () => {
    ...
    assert.ok(classesOf(item).includes("v-TitleBarButton"), "it wears the bar's face");
...
test("its menu drops from the item rather than pointing at it", () => {
    ...
    assert.equal(menu.alignLeft, true, "and its left edge lines up with the item's");
    assert.ok(
...
test("the item's action is what its menu chose, not the press", () => {
    const chosen = [];
    const {items, viewOf} = menuBar(1, {action: (item, value) => chosen.push(value)});
...
test("the chevron is off unless it is asked for", () => {
    ...
    assert.equal(
        menuBar(1, {showChevron: "true"}).items()[0].querySelectorAll("i.chevron").length,
        1,
