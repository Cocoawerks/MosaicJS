...
const {DeckView, Tab, TabView} = await import(
...
test("a tab view draws a bar of tabs over a deck of cards", () => {
    ...
    assert.deepEqual(buttons.map((b) => b.textContent), ["Overview", "Details", "Settings"]);
...
test("the pill is drawn behind the tabs, and does not slide into place", () => {
    ...
    assert.ok(classesOf(pill).includes("no-anim"), "there is nowhere to slide from");
