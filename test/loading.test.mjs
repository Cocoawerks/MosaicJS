...
const {LoadingButton, setLoadingDelay, getLoadingDelay, LoadingIndicator, Size, State} =
  await import(
...
test("draws a button with a loader in its icon slot", () => {
  ...
  assert.equal(spinner.style.visibility, "hidden", "and is not shown until it has to be");
  assert.equal(el.childNodes[0].childNodes[1].textContent, "Save", "the label is still there");
});
...
test("work that finishes quickly never shows a spinner", async () => {
  ...
  assert.equal(view.node.childNodes[0].childNodes[0].style.visibility, "hidden");
...
test("work that runs on shows one, once the wait has passed", async () => {
  ...
  assert.equal(view.node.childNodes[0].childNodes[0].style.visibility, "visible");
...
test("draws a spinner, and a message only when there is one", () => {
  ...
  assert.ok(classesOf(bare.el.childNodes[0]).includes("v-LoadingIndicator-spinner"));

  const spoken = open(LoadingIndicator, { message: "Loading…" });
  assert.equal(spoken.el.childNodes[1].textContent, "Loading…");
  assert.ok(classesOf(spoken.el.childNodes[1]).includes("v-LoadingIndicator-message"));
});
...
test("the size is a class, as it is in the Java version", () => {
  ...
  assert.equal(classesOf(view.node).includes("large"), false, "one size at a time");
});
...
test("finishing turns the spinner into a tick, and failing into a cross", () => {
  ...
  assert.ok(classesOf(view.node.childNodes[1]).includes("is-error"), "and the message reads as one");
