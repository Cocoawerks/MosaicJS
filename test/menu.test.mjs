...
const {Menu, MenuButton, MenuItem, Tooltip} = await import(
...
const activeText = (items) =>
    items.filter((li) => classesOf(li).includes("active")).map((li) => li.textContent)[0] ?? null;
...
test("but one dropped from a menu button points back at it", () => {
    ...
    const host = document.createElement("div");
    document.body.appendChild(host);
    const view = mount(MenuButton, host, {text: "Edit", children: someItems()}).view;
...
test("clicking an item chooses it, and puts the menu away", () => {
    const chosen = [];
    const {view, anchor, items} = menu({action: (m, value) => chosen.push(value)});
...
test("and an item may be answered on its own", () => {
    const said = [];
    const {view, items} = menu({}, [
        h(MenuItem, {text: "Cut", value: "cut", action: (m, value) => said.push(value)}),
    ]);
...
test("the arrows walk the lines that can be chosen, and stop at the ends", () => {
    ...
    assert.equal(view.activeValue, "copy");
    keyDown(el, "ArrowDown");
    assert.equal(view.activeValue, "copy", "the rule and the disabled line are passed over");
...
test("Enter chooses whatever is active", () => {
    const chosen = [];
    const {view, el, anchor} = menu({action: (m, value) => chosen.push(value)});
    view.alignWith(anchor);
...
test("a menu button says it has a menu, and has none until it is pressed", () => {
    ...
    assert.equal(el.textContent, "Edit", "and its children are the menu's, not its own");
});
