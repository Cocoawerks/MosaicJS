...
const { PopOver, PopOverOrientation, closeTransientPopOvers } =
...
test("it takes the side its orientation asked for, when there is room", () => {
  ...
  const { view, el } = against(
    { left: 400, top: 100, width: 80, height: 30 },
...
test("and the opposite side when that one has no room", () => {
  ...
  const { view, el } = against(
    { left: 400, top: 700, width: 80, height: 30 },
    { width: 200, height: 150 },
  );
  ...
  assert.ok(
    classesOf(calloutOf(el).childNodes[0]).includes("PopOver-triangle--t"),
  );
});
...
test("and left gives way to right", () => {
  const { view } = against(
    { left: 20, top: 300, width: 80, height: 30 },
    { width: 200, height: 150 },
    { orientation: PopOverOrientation.LEFT_MIDDLE },
  );
...
test("right gives way to left", () => {
  const { view } = against(
    { left: 900, top: 300, width: 80, height: 30 },
    { width: 200, height: 150 },
    { orientation: PopOverOrientation.RIGHT_MIDDLE },
  );
...
test("it stays where it was told when neither side fits", () => {
  ...
  const { view } = against(
    { left: 400, top: 300, width: 80, height: 30 },
...
test("a popover shown at a point takes the side it was told, having nothing to fit", () => {
  ...
  assert.equal(
    view.shownPosition,
    "top",
    "flipped while it hung from something",
  );
...
test("the callout is left out when it could not be aimed at the anchor", () => {
  ...
  const { view, el } = against(
    { left: 400, top: 760, width: 80, height: 30 },
