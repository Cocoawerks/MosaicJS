 ...
 *
 * Two SplitPanels are driven from here through their outlets: the bar's buttons
 * say what to do to them, and each panel reports back the length its divider was
...
export default class AppController {
  ...
  /**
   * Shut the sidebar away, or bring it back. The panel remembers how wide it
  ...
  /**
   * Turn the outer panel on its side. The panes keep everything else — which
  ...
  thicknessChanged(combo, value) {
    ...
    // Both panels have to place their panes again: the divider between them
  ...
  // --- what the panels report back -------------------------------------------

  /**
   * A divider was dragged. The panel passes the length it settled on, already
   * clamped to the bounds the markup gave it.
   *
   * @param {object} panel The SplitPanel that was dragged.
   * @param {number} length How long its static pane now is.
   */
  sidebarResized(panel, length) {
    this.sidebarWidth = String(Math.round(length));
    this.note = "dragging the sidebar";
  }

  previewResized(panel, length) {
