...
export default class Menu extends PopOver {
    ...
    alignWith(anchor, offsetLeft = 0, offsetTop = 0) {
        ...
        if (element) {
            ...
            const below = window.innerHeight - box.bottom >= (this.node?.offsetHeight ?? 0);
            const side = below ? "BOTTOM" : "TOP";
            this.orientation = PopOverOrientation[`${side}_${this.alignLeft ? "LEFT" : "CENTER"}`];
        }
    ...
    keyDown(event) {
        switch (event.key) {
            ...
            case "ArrowRight": {
                ...
                const item = this.items.find((entry) => entry.value === this.activeValue);
                if (!item?.submenu) return;
                event.preventDefault?.();
