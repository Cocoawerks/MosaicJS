...
export default class ToolBar extends Component {
    ...
    get overflowableItems() {
        return this.children.filter(
            (child) => child.type === ToolBarItem && child.props.hidden !== true
    ...
    itemElements() {
        ...
        return [...bar.childNodes].filter(
            (node) =>
                ...
                (node.getAttribute("class") ?? "").split(/\s+/).includes("v-ToolBarItem") &&
    ...
    get overflowedItems() {
        ...
        return this.overflowCount === 0 ? [] : items.slice(items.length - this.overflowCount);
    }
