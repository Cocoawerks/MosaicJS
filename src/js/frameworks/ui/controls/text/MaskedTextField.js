...
export default class MaskedTextField extends TextBase {
  ...
  parseMask() {
    ...
    for (let i = 0; i < mask.length; i++) {
      ...
      if (c === "?") {
        ...
        if (this.firstNonMaskPos === -1) this.firstNonMaskPos = this.tests.length - 1;
  ...
  placeholderCharAt(pos) {
    ...
    return pos < placeholder.length ? placeholder.charAt(pos) : placeholder.charAt(0);
  }

  /** The next position the user can type in, at or after `pos`. */
  seekNext(pos) {
    while (++pos < this.len && (pos >= this.tests.length || this.tests[pos] === null));
    return pos;
  }

  /** The last such position before `pos`. */
  seekPrev(pos) {
    while (--pos >= 0 && (pos >= this.tests.length || this.tests[pos] === null));
    return pos;
  }
  ...
  firstUnfilled() {
    for (let i = 0; i < this.len && i < this.buffer.length; i++) {
      if (this.tests[i] && this.buffer[i] === this.placeholderCharAt(i)) return i;
    }
    return this.len;
  }
  ...
  hasFocus() {
    return !!this.inputLayer && this.inputLayer === this.inputLayer.ownerDocument?.activeElement;
  }
  ...
  clearBuffer(start, end) {
    for (let i = start; i < end && i < this.len && i < this.buffer.length; i++) {
  ...
  shiftLeft(begin, end) {
    ...
    for (let i = begin, j = this.seekNext(end); i < this.len && i < this.buffer.length; i++) {
      if (!this.tests[i]) continue;
      if (j < this.len && j < this.buffer.length && this.tests[i].test(this.buffer[j])) {
  ...
  checkVal(allow, caretHint = -1) {
    ...
    if (allow) {
      ...
      if (current != null && lastMatch >= 0 && lastMatch + 1 <= current.length) {
    ...
    if (caretHint >= 0 && caretHint <= this.len) {
      ...
      if (hint < this.len && hint < this.tests.length && this.tests[hint] === null) {
