...
const PALETTE = [
    0x99c1f1, 0x8ff0a4, 0xf9f06b, 0xffbe6f, 0xf66151, 0xdc8add, 0xcdab8f, 0xffffff, 0x77767b,
    0x62a0ea, 0x57e389, 0xf8e45c, 0xffa348, 0xed333b, 0xc061cb, 0xb5835a, 0xf6f5f4, 0x5e5c64,
    0x3584e4, 0x33d17a, 0xf6d32d, 0xff7800, 0xe01b24, 0x9141ac, 0x986a44, 0xdeddda, 0x3d3846,
    0x1c71d8, 0x2ec27e, 0xf5c211, 0xe66100, 0xc01c28, 0x813d9c, 0x865e3c, 0xc0bfbc, 0x241f31,
    0x1a5fb4, 0x26a269, 0xe5a50a, 0xa51d2d, 0x8d1a1a, 0x613583, 0x63452c, 0x9a9996, 0x000000,
];
...
export default class ColorChooserPanel extends Component {
    ...
    seed() {
        ...
        const color =
            stated instanceof Color ? stated : Color.fromHex(stated ?? "") ?? Color.white();
        this.current = color;
        const hsb = color.toHSB();
        this.hue = hsb.hue;
        this.saturation = hsb.saturation;
    ...
    pickFrom(which, event) {
        ...
        if (which === "sv") {
            const saturation = clamp01((event.clientX - box.left) / Math.max(1, box.width - 1));
            const brightness =
        ...
        this.setColor(
            Color.fromHSB(this.hue, this.saturation, this.brightness, this.current.alpha),
        );
    }
    ...
    paintStrip() {
        ...
        for (let stop = 0; stop <= 6; stop++) {
            spectrum.addColorStop(stop / 6, Color.fromHSB(stop / 6, 1, 1).toHexString());
        }
        ...
        const y = Math.max(
            0,
            Math.min(height - barHeight, Math.round(this.hue * height) - barHeight / 2),
        );
        context.lineWidth = 1;
        context.strokeStyle = "#FFFFFF";
