...
export default class RangeSlider extends AbstractSlider {
    ...
    setValue(range, fireEvents = false) {
        ...
        this.endHandle.setValue(Math.min(this.maxValue, Number(range.end)), fireEvents);
        this.startHandle.setValue(Math.max(this.minValue, Number(range.start)), fireEvents);
    }
    ...
    handleFor(event) {
        ...
        const at = this.vertical ? event.clientY - rect.top : event.clientX - rect.left;
        const from = this.startHandle.calcPosition();
        ...
        return Math.abs(at - to) < Math.abs(at - from) ? this.endHandle : this.startHandle;
    }
    ...
    setActive(handle, active) {
        ...
        for (const other of this.handles) {
            if (other.element) other.element.style.zIndex = other === handle ? "1" : "0";
        }
    }
