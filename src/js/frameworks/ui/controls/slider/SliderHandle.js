...
export default class SliderHandle {
    ...
    calcValue(event, fireEvents) {
        ...
        const offsetX = Math.min(rect.width, event.clientX - rect.left) - this.grabOffset.x;
        const offsetY = Math.min(rect.height, event.clientY - rect.top) - this.grabOffset.y;

        this.setValue(this.valueOfPosition(this.slider.vertical ? offsetY : offsetX), fireEvents);
    }
