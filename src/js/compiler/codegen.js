...
import { ACTION_ATTR, STYLE_NAME_ATTR, VIEW_TAG, jsKey, jsString, lineMarker } from "./js.js";
...
export function generate(comp, opts) {
  const scopeAttr = `data-mosaic-${opts.hash}`;
  ...
  if (hasStyle) {
    ...
    out += `const ${cssVar} = ${jsString(css.scope(comp.style, scopeAttr))};\n`;
  ...
  const ctx = new Ctx(hasStyle ? scopeAttr : null);
...
class Ctx {
  /** `null` when the component has no styles — no attribute is emitted then. */
  constructor(scopeAttr) {
  ...
  propsExpr(tag, attrs, outlet, actions, isComponent) {
    ...
    for (const a of attrs) {
      ...
      const key = a.name === STYLE_NAME_ATTR && !isComponent ? "class" : a.name;
      let value;
      if (a.value.kind === "empty") {
        value = "true";
      } else if (a.value.kind === "static") {
        value = jsString(a.value.text);
      } else {
        const items = a.value.parts.map((p) =>
    ...
    // not scoped — their own file styles their own markup.
    if (!isComponent && tag.toLowerCase() !== "style" && this.scopeAttr) {
      entries.push(`${jsKey(this.scopeAttr)}: ""`);
    }
