...
/** The throwaway plugin that asks TypeDoc what it knows; see knownBlockTags. */
...
// the block tags this TypeDoc knows so that \`@fires\` can be added to them
// rather than replacing them. Loaded with --help, so nothing is converted.
export function load(app) {
  console.log("${DOC_ASK_MARKER}" + JSON.stringify(app.options.getValue("blockTags")));
}
`;
...
/**
 * The block tags TypeDoc knows, asked of TypeDoc itself.
 *
 * `blockTags` is a list that replaces the defaults rather than adding to them,
 * so declaring one more tag means naming all forty-odd — and a copy of that
 * list kept here would go stale the first time TypeDoc changed it, quietly
 * un-declaring whatever it had added. Setting the option from a plugin does
 * not work either: it takes, and the comments have already been parsed by the
 * time it does.
 *
 ...
 *
 * @returns {Promise<string[]|null>} The tags, or null if it could not be asked
 *   — in which case the tags are left alone and `@fires` goes on warning,
 *   which is a worse page rather than a failed run.
 */
async function knownBlockTags(command, dir) {
 ...
 */
function writeDocOptions(config, app, dir, docs, places, outdir, blockTags) {
  ...
  // with "Unknown option 'frameworks'".
  const { compilerOptions, include, frameworks,...options } = docs;
...
async function documentation(config, app, args) {
  ...
  const known = await knownBlockTags(command, app.outdir);
  const blockTags = known
    ? [...known,...DOC_BLOCK_TAGS.filter((tag) => !known.includes(tag))]
    : null;
  const options = writeDocOptions(
    config, app, app.outdir, docs, places, outdir, blockTags,
  );
